"use client"

import { useEffect, useMemo, useState } from "react"
import { apiFetchResponse, apiGet, apiPost } from "../../../lib/api-client"
import { useToast } from "../../../components/toast"
import { toastApiError } from "../../../lib/toast-api-error"
import { assignmentSummary, normalizeVolunteer, type NormalizedVolunteer } from "../../../lib/normalize-volunteer"
import { IconAction } from "../../../components/icon-action"
import {
    CheckCircle2,
    Download,
    Eye,
    Mail,
    MapPin,
    MessageSquare,
    Search,
    Send,
    UserPlus,
    XCircle
} from "lucide-react"

type Event = {
    url_slug: string
    title: string
    start_at: string
}

type AssignmentFormErrors = Partial<{
    volunteerSlug: string
    roleName: string
    shiftEnd: string
}>

type BroadcastFormErrors = Partial<{
    subject: string
    message: string
}>

export default function VolunteersPage() {
    const { toast } = useToast()
    const [volunteers, setVolunteers] = useState<NormalizedVolunteer[]>([])
    const [events, setEvents] = useState<Event[]>([])
    const [filters, setFilters] = useState({
        query: "",
        role: "all",
        assignment: "all" as "all" | "assigned" | "completed" | "unassigned"
    })
    const [selected, setSelected] = useState<NormalizedVolunteer | null>(null)

    const [assignmentForm, setAssignmentForm] = useState({
        volunteerSlug: "",
        eventSlug: "",
        roleName: "",
        status: "assigned",
        shiftStart: "",
        shiftEnd: "",
        location: ""
    })
    const [assignmentErrors, setAssignmentErrors] = useState<AssignmentFormErrors>({})

    const [broadcastForm, setBroadcastForm] = useState({
        subject: "",
        message: ""
    })
    const [broadcastErrors, setBroadcastErrors] = useState<BroadcastFormErrors>({})

    const handleExport = async (format: "csv" | "xlsx") => {
        try {
            const res = await apiFetchResponse(`/admin/volunteers/export?format=${format}`)
            const blob = await res.blob()
            const url = window.URL.createObjectURL(blob)
            const link = document.createElement("a")
            link.href = url
            link.download = `volunteers-export.${format}`
            document.body.appendChild(link)
            link.click()
            link.remove()
            window.URL.revokeObjectURL(url)
        } catch (err) {
            toastApiError(toast, err)
        }
    }

    const loadVolunteers = async () => {
        try {
            const data = await apiGet<any[]>("/admin/volunteers")
            const normalized = (data ?? []).map(normalizeVolunteer)
            setVolunteers(normalized)
            if (normalized.length > 0 && !assignmentForm.volunteerSlug) {
                setAssignmentForm((prev) => ({ ...prev, volunteerSlug: normalized[0].slug }))
            }
        } catch (err) {
            toastApiError(toast, err)
        }
    }

    const loadEvents = async () => {
        try {
            const data = await apiGet<Event[]>("/admin/events")
            setEvents(data)
        } catch (err) {
            toastApiError(toast, err)
        }
    }

    useEffect(() => {
        void loadVolunteers()
        void loadEvents()
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [])

    const validateAssignmentForm = () => {
        const next: AssignmentFormErrors = {}
        if (!assignmentForm.volunteerSlug.trim()) next.volunteerSlug = "Volunteer is required."
        if (!assignmentForm.roleName.trim()) next.roleName = "Role is required."

        if (assignmentForm.shiftStart && assignmentForm.shiftEnd) {
            const start = new Date(assignmentForm.shiftStart).getTime()
            const end = new Date(assignmentForm.shiftEnd).getTime()
            if (!Number.isNaN(start) && !Number.isNaN(end) && end <= start) {
                next.shiftEnd = "Shift end must be after shift start."
            }
        }

        setAssignmentErrors(next)
        return Object.keys(next).length === 0
    }

    const validateBroadcastForm = () => {
        const next: BroadcastFormErrors = {}
        if (!broadcastForm.message.trim()) next.message = "Message is required."
        if (broadcastForm.subject && broadcastForm.subject.trim().length > 140) {
            next.subject = "Subject is too long (max 140 chars)."
        }
        setBroadcastErrors(next)
        return Object.keys(next).length === 0
    }

    const handleAssign = async () => {
        if (!validateAssignmentForm()) {
            toast({ title: "Validation Error", description: "Please fix the highlighted fields.", variant: "destructive" })
            return
        }
        try {
            await apiPost(`/admin/volunteers/${assignmentForm.volunteerSlug}/assign`, {
                event_slug: assignmentForm.eventSlug || undefined,
                role_name: assignmentForm.roleName.trim(),
                status: assignmentForm.status,
                shift_start: assignmentForm.shiftStart ? new Date(assignmentForm.shiftStart).toISOString() : undefined,
                shift_end: assignmentForm.shiftEnd ? new Date(assignmentForm.shiftEnd).toISOString() : undefined,
                location: assignmentForm.location.trim() || undefined
            })
            toast({ title: "Success", description: "Assignment saved", variant: "success" })
            setAssignmentForm((prev) => ({
                volunteerSlug: prev.volunteerSlug,
                eventSlug: "",
                roleName: "",
                status: "assigned",
                shiftStart: "",
                shiftEnd: "",
                location: ""
            }))
            setAssignmentErrors({})
            await loadVolunteers()
        } catch (err) {
            toastApiError(toast, err)
        }
    }

    const handleBroadcast = async () => {
        if (!validateBroadcastForm()) {
            toast({ title: "Validation Error", description: "Please fix the highlighted fields.", variant: "destructive" })
            return
        }
        try {
            await apiPost("/admin/volunteers/communicate", {
                subject: broadcastForm.subject.trim() || undefined,
                message: broadcastForm.message.trim()
            })
            toast({ title: "Success", description: "Broadcast queued", variant: "success" })
            setBroadcastForm({ subject: "", message: "" })
            setBroadcastErrors({})
        } catch (err) {
            toastApiError(toast, err)
        }
    }

    const preferencesMap = useMemo(() => {
        return volunteers.reduce<Record<string, string>>((acc, volunteer) => {
            const summary = [
                volunteer.preferred_event_slug ? `Event: ${volunteer.preferred_event_slug}` : "",
                volunteer.preferred_roles.length ? `Roles: ${volunteer.preferred_roles.join(", ")}` : ""
            ]
                .filter(Boolean)
                .join(" • ")
            acc[volunteer.slug] = summary
            return acc
        }, {})
    }, [volunteers])

    const roleOptions = useMemo(() => {
        const set = new Set<string>()
        for (const v of volunteers) for (const role of v.preferred_roles) set.add(role)
        return Array.from(set).sort((a, b) => a.localeCompare(b))
    }, [volunteers])

    const filtered = useMemo(() => {
        const q = filters.query.trim().toLowerCase()
        return volunteers.filter((v) => {
            if (q) {
                const hay = `${v.name} ${v.email} ${v.phone ?? ""}`.toLowerCase()
                if (!hay.includes(q)) return false
            }
            if (filters.role !== "all" && !v.preferred_roles.includes(filters.role)) return false
            const summary = assignmentSummary(v.assignments)
            if (filters.assignment !== "all" && summary.status !== filters.assignment) return false
            return true
        })
    }, [volunteers, filters])

    const chipForAssignment = (v: NormalizedVolunteer) => {
        const s = assignmentSummary(v.assignments)
        if (s.status === "completed") {
            return (
                <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-[11px] font-semibold text-emerald-700">
          <CheckCircle2 className="h-3.5 w-3.5" />
          Completed
        </span>
            )
        }
        if (s.status === "assigned") {
            return (
                <span className="inline-flex items-center gap-1 rounded-full bg-sky-50 px-2 py-1 text-[11px] font-semibold text-sky-700">
          <UserPlus className="h-3.5 w-3.5" />
          Assigned
        </span>
            )
        }
        return (
            <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-700">
        <XCircle className="h-3.5 w-3.5" />
        Unassigned
      </span>
        )
    }

    return (
        <main className="space-y-6">
            <div>
                <h1 className="text-2xl font-semibold text-forest">Volunteers</h1>
                <p className="text-sm text-slate-600">Coordinate signups, assignments, and communications.</p>
            </div>

            <section className="flex flex-wrap items-center gap-3">
                <div className="relative">
                    <Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
                    <input
                        value={filters.query}
                        onChange={(e) => setFilters((p) => ({ ...p, query: e.target.value }))}
                        className="w-72 rounded-full border border-slate-200 py-2 pl-9 pr-4 text-sm"
                        placeholder="Search name/email/phone..."
                    />
                </div>

                <select
                    value={filters.role}
                    onChange={(e) => setFilters((p) => ({ ...p, role: e.target.value }))}
                    className="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-700"
                >
                    <option value="all">All roles</option>
                    {roleOptions.map((r) => (
                        <option key={r} value={r}>
                            {r}
                        </option>
                    ))}
                </select>

                <select
                    value={filters.assignment}
                    onChange={(e) => setFilters((p) => ({ ...p, assignment: e.target.value as any }))}
                    className="rounded-full border border-slate-200 px-4 py-2 text-sm text-slate-700"
                >
                    <option value="all">All assignments</option>
                    <option value="unassigned">Unassigned</option>
                    <option value="assigned">Assigned</option>
                    <option value="completed">Completed</option>
                </select>

                <div className="ml-auto flex items-center gap-2">
                    <IconAction label="Export CSV" onClick={() => void handleExport("csv")}>
                        <Download className="h-4 w-4" />
                    </IconAction>
                    <IconAction label="Export XLSX" onClick={() => void handleExport("xlsx")}>
                        <Download className="h-4 w-4" />
                    </IconAction>
                </div>
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-6 space-y-4">
                <h2 className="text-lg font-semibold text-slate-800">Assign volunteer</h2>

                <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1">
                        <select
                            value={assignmentForm.volunteerSlug}
                            onChange={(e) => {
                                setAssignmentForm({ ...assignmentForm, volunteerSlug: e.target.value })
                                if (e.target.value.trim()) setAssignmentErrors((p) => ({ ...p, volunteerSlug: "" }))
                            }}
                            className={`rounded-xl border px-3 py-2 text-sm ${
                                assignmentErrors.volunteerSlug ? "border-rose-300" : "border-slate-200"
                            }`}
                        >
                            {volunteers.map((volunteer) => (
                                <option key={volunteer.slug} value={volunteer.slug}>
                                    {volunteer.name} ({volunteer.email})
                                </option>
                            ))}
                        </select>
                        {assignmentErrors.volunteerSlug ? (
                            <p className="text-xs text-rose-600">{assignmentErrors.volunteerSlug}</p>
                        ) : null}
                    </div>

                    <select
                        value={assignmentForm.eventSlug}
                        onChange={(e) => setAssignmentForm({ ...assignmentForm, eventSlug: e.target.value })}
                        className="rounded-xl border border-slate-200 px-3 py-2 text-sm"
                    >
                        <option value="">Any event</option>
                        {events.map((event) => (
                            <option key={event.url_slug} value={event.url_slug}>
                                {event.title} ({event.start_at ? new Date(event.start_at).toLocaleDateString() : "TBD"})
                            </option>
                        ))}
                    </select>

                    <div className="space-y-1">
                        <input
                            value={assignmentForm.roleName}
                            onChange={(e) => {
                                setAssignmentForm({ ...assignmentForm, roleName: e.target.value })
                                if (e.target.value.trim()) setAssignmentErrors((p) => ({ ...p, roleName: "" }))
                            }}
                            className={`rounded-xl border px-3 py-2 text-sm ${
                                assignmentErrors.roleName ? "border-rose-300" : "border-slate-200"
                            }`}
                            placeholder="Role (e.g. Check-in)"
                        />
                        {assignmentErrors.roleName ? <p className="text-xs text-rose-600">{assignmentErrors.roleName}</p> : null}
                    </div>

                    <div className="relative">
                        <MapPin className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-slate-400" />
                        <input
                            value={assignmentForm.location}
                            onChange={(e) => setAssignmentForm({ ...assignmentForm, location: e.target.value })}
                            className="rounded-xl border border-slate-200 py-2 pl-9 pr-3 text-sm"
                            placeholder="Location"
                        />
                    </div>

                    <label className="text-xs text-slate-500">
                        Shift start
                        <input
                            type="datetime-local"
                            value={assignmentForm.shiftStart}
                            onChange={(e) => setAssignmentForm({ ...assignmentForm, shiftStart: e.target.value })}
                            className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm"
                        />
                    </label>

                    <label className="text-xs text-slate-500">
                        Shift end
                        <input
                            type="datetime-local"
                            value={assignmentForm.shiftEnd}
                            onChange={(e) => {
                                setAssignmentForm({ ...assignmentForm, shiftEnd: e.target.value })
                                setAssignmentErrors((p) => ({ ...p, shiftEnd: "" }))
                            }}
                            className={`mt-1 w-full rounded-xl border px-3 py-2 text-sm ${
                                assignmentErrors.shiftEnd ? "border-rose-300" : "border-slate-200"
                            }`}
                        />
                        {assignmentErrors.shiftEnd ? <p className="mt-1 text-xs text-rose-600">{assignmentErrors.shiftEnd}</p> : null}
                    </label>
                </div>

                <button
                    onClick={() => void handleAssign()}
                    className="inline-flex items-center gap-2 rounded-full bg-forest px-4 py-2 text-sm font-semibold text-white"
                >
                    <UserPlus className="h-4 w-4" />
                    Save assignment
                </button>
            </section>

            <section className="rounded-2xl border border-slate-200 bg-white p-6 space-y-4">
                <h2 className="text-lg font-semibold text-slate-800">Volunteer broadcast</h2>

                <div className="space-y-1">
                    <input
                        value={broadcastForm.subject}
                        onChange={(e) => {
                            setBroadcastForm({ ...broadcastForm, subject: e.target.value })
                            setBroadcastErrors((p) => ({ ...p, subject: "" }))
                        }}
                        className={`w-full rounded-xl border px-3 py-2 text-sm ${
                            broadcastErrors.subject ? "border-rose-300" : "border-slate-200"
                        }`}
                        placeholder="Subject (optional)"
                    />
                    {broadcastErrors.subject ? <p className="text-xs text-rose-600">{broadcastErrors.subject}</p> : null}
                </div>

                <div className="space-y-1">
          <textarea
              value={broadcastForm.message}
              onChange={(e) => {
                  setBroadcastForm({ ...broadcastForm, message: e.target.value })
                  if (e.target.value.trim()) setBroadcastErrors((p) => ({ ...p, message: "" }))
              }}
              className={`w-full rounded-xl border px-3 py-2 text-sm ${
                  broadcastErrors.message ? "border-rose-300" : "border-slate-200"
              }`}
              rows={3}
              placeholder="Write the message to all volunteers."
          />
                    {broadcastErrors.message ? <p className="text-xs text-rose-600">{broadcastErrors.message}</p> : null}
                </div>

                <button
                    onClick={() => void handleBroadcast()}
                    className="inline-flex items-center gap-2 rounded-full border border-forest/30 px-4 py-2 text-sm font-semibold text-forest"
                >
                    <Send className="h-4 w-4" />
                    Send broadcast
                </button>
            </section>

            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-xs uppercase text-slate-500">
                    <tr>
                        <th className="px-4 py-3">Name</th>
                        <th className="px-4 py-3">Preferences</th>
                        <th className="px-4 py-3">Assignments</th>
                        <th className="px-4 py-3">Created</th>
                        <th className="px-4 py-3">Actions</th>
                    </tr>
                    </thead>
                    <tbody>
                    {filtered.map((volunteer) => (
                        <tr key={volunteer.slug} className="border-t border-slate-100 align-top">
                            <td className="px-4 py-3">
                                <div className="flex items-center justify-between gap-2">
                                    <div className="font-medium text-slate-800">{volunteer.name}</div>
                                    {chipForAssignment(volunteer)}
                                </div>
                                <div className="text-xs text-slate-500">{volunteer.email}</div>
                                <div className="text-xs text-slate-500">{volunteer.phone || "-"}</div>
                            </td>

                            <td className="px-4 py-3 text-xs text-slate-600">
                                <div className="space-y-2">
                                    <div>{preferencesMap[volunteer.slug] || "No preferences submitted"}</div>
                                    <div className="flex flex-wrap gap-1.5">
                                        {volunteer.preferred_roles.map((role) => (
                                            <span key={role} className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-700">
                          {role}
                        </span>
                                        ))}
                                    </div>
                                </div>
                            </td>

                            <td className="px-4 py-3 text-xs text-slate-600">
                                {volunteer.assignments.length > 0 ? (
                                    <div className="space-y-1">
                                        {volunteer.assignments.slice(0, 3).map((assignment, index) => (
                                            <div key={`${assignment.role_name}-${index}`}>
                                                {assignment.role_name}
                                                {assignment.event_title ? ` • ${assignment.event_title}` : ""}
                                                {assignment.status ? ` • ${assignment.status}` : ""}
                                            </div>
                                        ))}
                                        {volunteer.assignments.length > 3 ? <div>+{volunteer.assignments.length - 3} more</div> : null}
                                    </div>
                                ) : (
                                    "No assignments yet"
                                )}
                            </td>

                            <td className="px-4 py-3 text-xs text-slate-600">
                                {volunteer.created_at ? volunteer.created_at.toLocaleDateString() : "-"}
                            </td>

                            <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                    <IconAction label="View details" onClick={() => setSelected(volunteer)}>
                                        <Eye className="h-4 w-4" />
                                    </IconAction>
                                    <IconAction
                                        label="Assign"
                                        onClick={() => {
                                            setAssignmentForm((p) => ({ ...p, volunteerSlug: volunteer.slug, roleName: "" }))
                                            window.scrollTo({ top: 0, behavior: "smooth" })
                                        }}
                                    >
                                        <UserPlus className="h-4 w-4" />
                                    </IconAction>
                                    <IconAction label="Email" href={`mailto:${encodeURIComponent(volunteer.email)}`}>
                                        <Mail className="h-4 w-4" />
                                    </IconAction>
                                    <IconAction
                                        label="Quick message"
                                        onClick={() => {
                                            setBroadcastForm((p) => ({
                                                ...p,
                                                subject: p.subject || `Volunteer: ${volunteer.name}`,
                                                message: p.message
                                            }))
                                            toast({ title: "Tip", description: "Broadcast is for all volunteers. Use Email for a single person.", variant: "default" })
                                        }}
                                    >
                                        <MessageSquare className="h-4 w-4" />
                                    </IconAction>
                                </div>
                            </td>
                        </tr>
                    ))}

                    {filtered.length === 0 && (
                        <tr>
                            <td colSpan={5} className="px-4 py-6 text-center text-slate-500">
                                No volunteers yet.
                            </td>
                        </tr>
                    )}
                    </tbody>
                </table>
            </div>

            {selected ? (
                <div className="fixed inset-0 z-50 bg-black/40 p-4" role="dialog" aria-modal="true">
                    <div className="mx-auto max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-900">{selected.name}</h3>
                                <p className="text-sm text-slate-600">{selected.email}</p>
                                <p className="text-sm text-slate-600">{selected.phone || "-"}</p>
                            </div>
                            <button
                                className="inline-flex items-center gap-2 rounded-full border border-slate-200 px-3 py-1.5 text-sm font-semibold text-slate-700"
                                onClick={() => setSelected(null)}
                            >
                                <XCircle className="h-4 w-4" />
                                Close
                            </button>
                        </div>

                        <div className="mt-4 space-y-4 text-sm">
                            <div>
                                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Preferences</p>
                                <p className="mt-1 text-slate-700">{preferencesMap[selected.slug] || "No preferences submitted"}</p>
                            </div>

                            <div>
                                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Assignments</p>
                                {selected.assignments.length ? (
                                    <div className="mt-2 space-y-2">
                                        {selected.assignments.map((a, idx) => (
                                            <div key={`${a.role_name}-${idx}`} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                                                <div className="font-semibold text-slate-800">{a.role_name}</div>
                                                <div className="text-slate-600">
                                                    {(a.event_title ?? "Any event") + (a.status ? ` • ${a.status}` : "")}
                                                </div>
                                                {a.shift_start ? (
                                                    <div className="text-xs text-slate-500">
                                                        {a.shift_start.toLocaleString()}
                                                        {a.shift_end ? ` → ${a.shift_end.toLocaleString()}` : ""}
                                                        {a.location ? ` • ${a.location}` : ""}
                                                    </div>
                                                ) : a.location ? (
                                                    <div className="text-xs text-slate-500">{a.location}</div>
                                                ) : null}
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="mt-1 text-slate-600">No assignments yet.</p>
                                )}
                            </div>

                            {selected.notes ? (
                                <div>
                                    <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Notes</p>
                                    <p className="mt-1 whitespace-pre-wrap text-slate-700">{selected.notes}</p>
                                </div>
                            ) : null}
                        </div>
                    </div>
                </div>
            ) : null}
        </main>
    )
}
